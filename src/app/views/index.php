<!DOCTYPE html>
<html lang="es">

<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<meta http-equiv="X-UA-Compatible" content="ie=edge">
	<title>WeekFood</title>
	<link rel="stylesheet" href="css/estilo.css">
	<link rel="shortcut icon" href="imagenes/WeekFoodLogo_512x512.png" type="image/x-icon">
	<link rel="stylesheet" href="fontawesome/css/all.css">
</head>

<body class="p-principal">
	<div class="l-distribucion">
		<div class="l-distribucion__cabecera">
			<div class="c-cabecera">
				<div class="c-cabecera__boton-menu js-boton-menu">
					<i class="fas fa-bars"></i>
				</div>
				<div class="c-cabecera__logo">
					<h1 class="c-logo">
						<span class="c-logo__parte">W</span>
						<span class="c-logo__parte c-logo__parte--alterna">F</span>
					</h1>
				</div>
				<div class="c-cabecera__logo c-cabecera__logo--completo">
					<h1 class="c-logo">
						<span class="c-logo__parte">Week</span>
						<span class="c-logo__parte c-logo__parte--alterna">Food</span>
					</h1>
				</div>
				<div class="c-cabecera__botones">
					<i class="fas fa-user"></i>
					<i class="fas fa-shopping-cart"></i>
				</div>
			</div>
		</div>
		<div class="l-distribucion__menu">
			<div class="c-menu c-menu--oculto js-menu">
				<ul class='js-menu__lista c-menu__lista c-menu__lista--oculto'>
				</ul>
				<div class="js-menu__flecha c-menu__slider">
					<i class="fas fa-2x fa-caret-right c-menu__flecha"></i>
					<i class="fas fa-2x fa-caret-left c-menu__flecha c-menu__flecha--oculto"></i>
				</div>
			</div>
		</div>
		<div class="l-distribucion__principal">
		</div>
		<div class="l-distribucion__pie">
			<div class="c-pie">
				<p>&copy; WeekFood 2018 - 2019</p>
			</div>
		</div>
	</div>
	<script src="js/libs/jquery-3.3.1.min.js">
	</script>
	<script src="js/bundle.js">
	</script>
</body>

</html>